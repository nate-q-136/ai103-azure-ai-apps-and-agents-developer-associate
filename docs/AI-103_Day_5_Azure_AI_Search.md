...
- [ ] Nêu đúng luồng dữ liệu: Data Source → Indexer → Skillset → Index.
- [ ] Phân biệt chính xác các thuộc tính trường: `searchable`, `filterable`, `sortable`, `facetable`, `retrievable`.
- [ ] Giải thích được sự khác nhau và cách thức hoạt động của Hybrid Search kết hợp thuật toán RRF.
- [ ] Hiểu rõ vai trò của Semantic Ranker trong việc trích xuất Semantic Captions và Answers.
- [ ] Nắm vững quy tắc chi phí: Dùng Free Tier F1 để tiết kiệm budget, tuyệt đối không để quên resource Basic/Standard qua đêm.
